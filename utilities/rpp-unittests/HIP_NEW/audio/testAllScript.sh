...
if [[ "$1" -lt 1 ]] | [[ "$1" -gt 4 ]]; then
    echo "The starting case# must be in the 1-4 range!"
    echo
    ...
    echo "S     CASE_START (Starting case# (1-4))"
    echo "E     CASE_END (Ending case# (1-4))"
    exit 1
fi

if [[ "$2" -lt 1 ]] | [[ "$2" -gt 4 ]]; then
    echo "The ending case# must be in the 1-4 range!"
    echo
    echo "The testAllScript.sh bash script runs the RPP audio unittest testsuite for AMDRPP functionalities in HIP/OCL/HIP backends."
    echo
    echo "Syntax: ./testAllScriptAudio.sh <S> <E>"
