    ...
    for (int perfRunCount = 0; perfRunCount < 100; perfRunCount++)
    {
        ...
        case 46:
        {
            ...
            if (ip_bitDepth == 0)
                ...
                rppt_vignette_gpu(d_inputf16, srcDescPtr, d_outputf16, dstDescPtr, intensity, roiTensorPtrSrc, roiTypeSrc, handle);
            else if (ip_bitDepth == 2)
                rppt_vignette_gpu(d_inputf32, srcDescPtr, d_outputf32, dstDescPtr, intensity, roiTensorPtrSrc, roiTypeSrc, handle);
            else if (ip_bitDepth == 3)
                missingFuncFlag = 1;
            else if (ip_bitDepth == 4)
                missingFuncFlag = 1;
            else if (ip_bitDepth == 5)
                rppt_vignette_gpu(d_inputi8, srcDescPtr, d_outputi8, dstDescPtr, intensity, roiTensorPtrSrc, roiTypeSrc, handle);
            else if (ip_bitDepth == 6)
                missingFuncFlag = 1;
            else
                missingFuncFlag = 1;
