...
#pragma omp parallel for num_threads(dstDescPtr->n)
    for(int batchCount = 0; batchCount < dstDescPtr->n; batchCount++)
    {
        ...
        __m128 pAdj = _mm_set1_ps(adjustmentValue);
        ...
        if ((srcDescPtr->c == 3) && (srcDescPtr->layout == RpptLayout::NHWC) && (dstDescPtr->layout == RpptLayout::NCHW))
        {
            ...
            for(int i = 0; i < roi.xywhROI.roiHeight; i++)
            {
                ...
                for (; vectorLoopCount < alignedLength; vectorLoopCount += 12)
                {
                    ...
                    compute_color_temperature_12_host(p, pAdj);    // color_temperature adjustment
        ...
        if ((srcDescPtr->c == 3) && (srcDescPtr->layout == RpptLayout::NHWC) && (dstDescPtr->layout == RpptLayout::NCHW))
        {
            ...
            for(int i = 0; i < roi.xywhROI.roiHeight; i++)
            {
                ...
                for (; vectorLoopCount < bufferLength; vectorLoopCount += 3)
                {
                    ...
                    *dstPtrTempR = (Rpp8s) RPPPIXELCHECKI8(srcPtrTempI8[0] - adjustmentValue - 128);
                    *dstPtrTempG = (Rpp8s) RPPPIXELCHECKI8(srcPtrTempI8[1] - 128);
                    *dstPtrTempB = (Rpp8s) RPPPIXELCHECKI8(srcPtrTempI8[2] + adjustmentValue - 128);
        ...
        else if ((srcDescPtr->c == 3) && (srcDescPtr->layout == RpptLayout::NCHW) && (dstDescPtr->layout == RpptLayout::NHWC))
        {
            ...
            for(int i = 0; i < roi.xywhROI.roiHeight; i++)
            {
                ...
                for (; vectorLoopCount < bufferLength; vectorLoopCount++)
                {
                    ...
                    dstPtrTemp[0] = (Rpp8s) RPPPIXELCHECKI8(srcPtrTempI8[0] - adjustmentValue - 128);
                    dstPtrTemp[1] = (Rpp8s) RPPPIXELCHECKI8(srcPtrTempI8[1] - 128);
                    dstPtrTemp[2] = (Rpp8s) RPPPIXELCHECKI8(srcPtrTempI8[2] + adjustmentValue - 128);
